<?php
class ControllerPdThanhtoan extends Controller {
	public function index() {
		...
		$ts_history = $this -> model_pd_registercustom -> get_count_customer_history();
		...
		$pagination -> url = $this -> url -> link('pd/thanhtoan', 'page={page}&token='.$this->session->data['token'].'', 'SSL');
		
		$data['customer'] =  $this-> model_pd_registercustom->get_all_customer_history($limit, $start);
		
		$data['pagination'] = $pagination -> render();


		$this->response->setOutput($this->load->view('pd/thanhtoan.tpl', $data));
	}
	...
	public function load_barcode()
	{
		...
		$this -> load -> model('pd/registercustom');
		$get_name_customer = $this -> model_pd_registercustom -> get_barcode_customer_id($barcode);
