...
class ModelPdRegistercustom extends Model {
	...
	public function createPD($customer_id, $amount,$doanhso,$loinhuan){
		$date_added= date('Y-m-d H:i:s');
		$date_finish_ds = strtotime ( '+ '.$doanhso.' month' , strtotime ($date_added));
		$date_finish_ds= date('Y-m-d H:i:s',$date_finish_ds) ;
	...
	public function addCustomer($data) {
		...
		$this -> db -> query("
			...
			date_birth = '" . $this -> db -> escape($data['birthday']) . "', 
			address = '" . $this -> db -> escape($data['address']) . "', 
			firstname = '" . $this -> db -> escape($data['firstname']) . "', 
			nhanvientao = '" . $this -> db -> escape($data['nhanvien']) . "', 
			thongtin = '" . $this -> db -> escape($data['thongtin']) . "', 
			dichvu = '" . $this -> db -> escape($dichvu) . "', 
			ghichu = '" . $this -> db -> escape($data['ghichu']) . "', 
			status = '1', 
			giatien = '" . $this -> db -> escape(str_replace(",", "", $data['giatien'])) . "', 
			date_added = NOW()
			
		");
		$customer_id = $this -> db -> getLastId();
		$this -> db -> query("
			INSERT INTO " . DB_PREFIX . "history_customer SET
			customer_id = '".$customer_id."',
			nhanvientao = '" . $this -> db -> escape($data['nhanvien']) . "', 
			thongtintuvan = '" . $this -> db -> escape($data['thongtin']) . "', 
			dichvu = '" . $this -> db -> escape($dichvu) . "', 
			ghichu = '" . $this -> db -> escape($data['ghichu']) . "', 
			giatien = '" . $this -> db -> escape(str_replace(",", "", $data['giatien'])) . "', 
			date_added = NOW()
	...
	public function create_bill($data,$customer_id) {
		$dichvu = explode("_", $data['dichvu']);
		$dichvu = $dichvu[0];
		$this -> db -> query("
			INSERT INTO " . DB_PREFIX . "history_customer SET
	...
	public function removeCustomer($customer_id) {
		$this -> db -> query("
			DELETE FROM " . DB_PREFIX . "customer
			WHERE customer_id = '".$customer_id."'
	...
	public function getCustomLike_telephone($name) {
		$listId = '';
		$query = $this -> db -> query("
			SELECT telephone,firstname FROM ". DB_PREFIX ."customer
			WHERE telephone Like '%".$this->db->escape($name)."%'
			LIMIT 30
		") ;
		$array_id = $query -> rows;
	...
	public function getCustomLike_tuoi($age) {
		$listId = '';
		$query = $this -> db -> query("
			SELECT date_birth,customer_id FROM ". DB_PREFIX ."customer
		") ;
		$array_id = $query -> rows;
		$customer_id = "";
		...
		foreach ($array_id as $value) {
			...
			if ($this -> getAge(date('Y-m-d',strtotime($value['date_birth']))) == $age)
			{
				$customer_id .= ",".$value['customer_id'];
			}
		}
		$check_customer = explode(",", $customer_id);
		if (count($check_customer) > 1)
		{
			$querys = $this -> db -> query("
				SELECT * FROM ". DB_PREFIX ."customer WHERE customer_id IN (".substr($customer_id, 1).");
			") ;
			$array_ids = $querys -> rows;
	...
	public function getCustomer_print($id) {
		$query = $this -> db -> query("SELECT C.firstname,C.date_birth,C.telephone,C.address,D.* FROM " . DB_PREFIX . "customer C 
			INNER JOIN " . DB_PREFIX . "history_customer D ON C.customer_id = D.customer_id
		 WHERE D.id = '" . (int)$id . "'");
		return $query -> row;
	}
	...
	public function get_name_customer($firstname){
		$query = $this -> db -> query("
			SELECT *
			FROM  ".DB_PREFIX."customer A 
			WHERE A.firstname LIKE '%".$this -> db -> escape($firstname)."%'
		");
		return $query -> rows;
	}
	...
	public function get_barcode_customer_id($barcode){
		$query = $this -> db -> query("
			SELECT *
			FROM  ".DB_PREFIX."customer
			WHERE bar_code = '".$this -> db -> escape($barcode)."'
		");
		return $query -> row;
	}

	public function get_thang_customer_id($thangs){

		$query = $this -> db -> query("
			SELECT date_birth,customer_id
			FROM  ".DB_PREFIX."customer
		");
		$array = $query -> rows;
		$customer_id = "";
		foreach ($array as $value) {
			$thang = explode("-", $value['date_birth']);
		
			if (intval($thang[1]) == intval($thangs))
			{
				$customer_id .= ",".$value['customer_id'];
			}
		}
		...
		if (count($check_customer) > 1)
		{
			$query = $this -> db -> query("
				SELECT *
				FROM  ".DB_PREFIX."customer
				WHERE customer_id IN (".substr($customer_id, 1).");
			");
	...
	public function getCustomLike_name($name) {
		$listId = '';
		$query = $this -> db -> query("
			SELECT firstname,telephone,customer_id,date_birth FROM ". DB_PREFIX ."customer
			WHERE firstname Like '%".$this->db->escape($name)."%' OR telephone Like '%".$this->db->escape($name)."%'
			LIMIT 30
		") ;
		$array_id = $query -> rows;
	...
	public function get_packege_lager_doanhso($package)
	{
		$query = $this -> db -> query("
			SELECT *,MAX(filled) as filled
			FROM  ".DB_PREFIX."customer_provide_donation
			WHERE filled >= '".$package."' AND date_finish_ds >= NOW()
			GROUP BY customer_id
		");
		return $query -> rows;
	}
	...
	{
		$query = $this -> db -> query("
			SELECT * FROM   ".DB_PREFIX."dichvu ORDER BY id DESC
		");
		return $query -> rows;
	}
	public function get_dichvu_id($id)
	{
		$query = $this -> db -> query("
			SELECT * FROM   ".DB_PREFIX."dichvu WHERE id = '".$id."'
		");
		return $query -> row;
	}
	public function remove_dichvu_id($id)
	{
		$query = $this -> db -> query("
			DELETE FROM   ".DB_PREFIX."dichvu WHERE id = '".$id."'
		");
		return $query;
	}
	public function create_dichvu($data)
	{
		$query = $this -> db -> query("
			INSERT ".DB_PREFIX."dichvu SET
			name = '".$data['name']."',
	...
	public function update_dichvu_id($id,$data)
	{
		$query = $this -> db -> query("
			UPDATE ".DB_PREFIX."dichvu SET
			name = '".$data['name']."',
			price = '".str_replace(",","",$data['price'])."',
			date_added = NOW()
			WHERE id = '".$id."'
	...
	{
		$query = $this -> db -> query("
			SELECT * FROM   ".DB_PREFIX."nhanvien ORDER BY id DESC
		");
		return $query -> rows;
	}
	public function create_nhanvien($data)
	{
		$query = $this -> db -> query("
			INSERT ".DB_PREFIX."nhanvien SET
			name = '".$data['firstname']."',
			...
			luong = '".str_replace(",","",$data['tienluong'])."',
			date_added = NOW()
		");
		return $query ;
	}
	...
	{
		$query = $this -> db -> query("
			SELECT * FROM   ".DB_PREFIX."nhanvien WHERE id = '".$id."'
		");
		return $query -> row;
	}
	public function update_nhanvien_id($id,$data)
	{
		$query = $this -> db -> query("
			UPDATE ".DB_PREFIX."nhanvien SET
			name = '".$data['firstname']."',
			date_birthday = '".$data['birthday']."',
			telephone = '".$data['telephone']."',
			address = '".$data['address']."',
	...
	public function remove_nhanvien_id($id)
	{
		$query = $this -> db -> query("
			DELETE FROM   ".DB_PREFIX."nhanvien WHERE id = '".$id."'
		");
		return $query;
	}

	public function get_history_customer($customer_id){
		$query = $this -> db -> query("
			SELECT * FROM   ".DB_PREFIX."history_customer WHERE customer_id = '".$customer_id."'
		");
		return $query -> rows;
	}
	public function get_count_customer_history(){

		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer A INNER JOIN ".DB_PREFIX."history_customer B ON A.customer_id = B.customer_id
		");
		return $query -> row;
	}
	public function get_all_customer_history($limit, $offset){

		$query = $this -> db -> query("
			SELECT A.firstname,A.date_birth,A.telephone,B.thongtintuvan,B.dichvu,B.nhanvientao,B.giatien,B.date_added
			FROM  ".DB_PREFIX."customer A INNER JOIN ".DB_PREFIX."history_customer B ON A.customer_id = B.customer_id
			ORDER BY B.date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
	...
	public function get_count_customer_id_history($customer_id){

		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer A INNER JOIN ".DB_PREFIX."history_customer B ON A.customer_id = B.customer_id WHERE A.customer_id = '".$customer_id."'
		");
		return $query -> row;
	}
	public function get_all_customer_id_history($customer_id,$limit, $offset){

		$query = $this -> db -> query("
			SELECT A.firstname,A.date_birth,A.telephone,B.thongtintuvan,B.dichvu,B.nhanvientao,B.giatien,B.date_added
			FROM  ".DB_PREFIX."customer A INNER JOIN ".DB_PREFIX."history_customer B ON A.customer_id = B.customer_id WHERE B.customer_id = '".$customer_id."'
			ORDER BY B.date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function check_barcode($bar_code)
	{
		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer A WHERE A.bar_code = '".$bar_code."'
		");
		return $query -> row['number'];
	}
}
