...
//////////////////////////////////////
	...
	public function checklogin()
	{
		...
		if ($customer_info) {
			$code_active = $this -> model_customize_register -> get_code_active($customer_info['customer_id']);
			if (intval($code_active['number']) === 0) {
				return  "Xin vui lòng kích hoạt tài khoản của bạn!";
			}
		}
		if ($customer_info) {
			$code_active = $this -> model_customize_register -> get_code_active($customer_info['customer_id']);
		...
		if (!$this->error) {
			if (call_user_func_array("myHasLogin", array($this->request->post['email'], $this->request->post['password'], $this->customer))) {
				return  "Lỗi đăng nhập. Xin vui lòng thử lại";
				$this->model_account_customer->addLoginAttempt($this->request->post['email']);
			} else {
				$this->model_account_customer->deleteLoginAttempts($this->request->post['email']);
				$this->event->trigger('post.customer.login');
	...
	public function login_ajax()
	{
		$this -> load ->model('account/customer');
		$this -> load ->model('customize/register');
		if ($this -> request ->post['email'] == "" || $this -> request ->post['password'] == "" || $this -> request ->post['capcha'] == "")
		{
			$json['warning'] = "Vui lòng nhập đủ thông tin đăng nhập!";
		}
		else
		{
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {

				$json['warning'] = $this -> checklogin();
			}
		}
