...
$(function(){
    ...
    var _latitude = 10.7795835;
    var _longitude = 106.64823909999996;
    ...
    if( document.getElementById(element) != null ){
        ...
        if( optimizedDatabaseLoading == 1 ){
            google.maps.event.addListener(map, 'idle', function(){
                if( searchClicked != 1 ){
                   ...
                   loadData("index.php?route=account/dashboard/get_map_json", ajaxData);
                }
            });
        }
        else {
            google.maps.event.addListenerOnce(map, 'idle', function(){
                loadData("index.php?route=account/dashboard/get_map_json");
            });
        }
        ...
        function placeMarkers(markers){
            ...
            for (i = 0; i < markers.length; i++) {
                ...
                if( markers[i]["images"] != undefined ){
                    thumbnailImage = markers[i]["images"];
                }
                else {
                    thumbnailImage = "/ubcc_website/static/img/app-25.png";
                }

                if( markers[i]["featured"] == 1 ){
                    markerContent.innerHTML =
                    '<div class="marker" data-id="'+ markers[i]["id"] +'" data-lat="'+ markers[i]["lat"] +'" data-id="'+ markers[i]["lng"] +'">' +
                ...
                else if ( markers[i]["address"] && !markers[i]["lat"] && !markers[i]["lng"] ){
                    renderRichMarker(i,"address");
                }

                // Only Latitude and Longitude

                else if ( markers[i]["lat"] && markers[i]["lng"] && !markers[i]["address"] ) {
                    renderRichMarker(i,"latitudeLongitude");
                }
            ...
            function openSidebarDetail(id){
                $.ajax({
                    url : "index.php?route=account/dashboard/get_shop_id",
                    type : "post",
                    dateType:"text",
                    data : {
                         'id' : id
                    ...
                    success : function (result){
                        ...
                        $('.detail-marker').show();
                        bg_image =  results.images;
                        var shtml = '<div class="result-item-detail"> <div class="image" style="background-image: url('+bg_image+')"></div> <div class="description"> <h3>'+results.title+'</h3> <h5><i class="fa fa-map-marker"></i>'+results.address+'</h5> <p>'+results.description+'</p>  </div> </div> </a><a href="#" data-rel="back" class="close-box-marker"><i class="fa fa-times"></i></a>';

                        $(".info-marker").html(shtml);
                        ...
                        $(".close-box-marker").click(function(){
            ...
            // var clusterStyles = [
            //     {
            //         url: '/ubcc_website/static/img/cluster.png',
            //         height: 36,
            //         width: 36
            //     }
            // ];
            ...
            function renderResults(){
                ...
                if( $(".hero-section").hasClass("sidebar-grid") ){
                    ...
                    sidebarUrl = "http://199.193.6.16:8069/get-all-latlng";
                }

                $("#search_property_headers").keyup(function() {
                    ...
                    var icon_imagess;
                    var contentss;
                    openerp.jsonRpc('/search-category', 'call', {
                        post: $('#search_property_headers').val()
                        ...
                        $.each(result, function(key, item) {
                            ...
                            icon_images_s = "/img/shop/ubcc.pd.point/" + item['id'] + "/logo_shops/64x64";
                            html += '<li class="result-item position_marker" data-lat="'+lat+'" data-lng="'+lng+'" data-id="'+item['id']+'"><div class="info-block block-info clearfix"><div class="square-box pull-right"><img src="' + icon_images_s + '"/></div><p><span>' + item['title'] + '</span></p><p>' + item['address'] + '</p><p> <a href="http://' + item['domain'] + '" target="_blank">' + item['domain'] + '</a></p></div></li>';
                            lats = item['lat'];
                            lngs = item['lng'];
                            icon_imagess = "/img/shop/ubcc.pd.point/5/logo_shops/64x64";
                            contentss = '<div class="main-custom-hanel"><div class="left-image-hanel"> <img src="' + icon_imagess + '"/><div class="right-custom-hanel"><div id="hook" style=""><div class="detail-name"></div></div></div></div>'
                        ...
                        //     var Nlatlng = this.id;
                        //     Nlatlng = Nlatlng.replace(/\s/g, '');
                        //     var str_new = Nlatlng.split(",");
                        //     var new_lat = str_new[0];
                        //     var new_lng = str_new[1];
                        //     moveToLocation(new_lat, new_lng);
                        //   });
                        // ======================================================================
...
function autoComplete(map, marker){
    if( $("#search_property_header").length ){
        if( !map ){
            map = new google.maps.Map(document.getElementById("search_property_header"));
        }
        ...
        $(".geo-location").on("click", function() {
            if (navigator.geolocation) {
                $('#map-homepage').addClass('fade-map');
                navigator.geolocation.getCurrentPosition(success);
            } else {
                console.log('Geo Location is not supported');
            }
