...
class ModelAccountPd extends Model {
	...
	public function getInvoceFormHash($invoice_id_hash, $customer_id){
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX ."customer_invoice_lottery
	...
	public function updateConfirm($invoice_id_hash, $confirmations,$url){
		$query = $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer_invoice_lottery SET
	...
	public function getPD_lottery($id){
		$query = $this -> db -> query("
			SELECT * 
			FROM ". DB_PREFIX . "customer_provide_donation_lottery
			WHERE id = ".$id."
		");
		return $query -> row;
	}
	...
	public function updateDatefinishPD($pd_id,$max_profit){
		...
		if ($query -> row['amount_usd'] < 50000000)
		{
			$query = $this -> db -> query("
	...
	public function saveInvoice($customer_id, $secret,$invoice_id_hash, $amount, $pd_id,$my_wallet,$callback){
		$query = $this -> db -> query("
			INSERT INTO ".DB_PREFIX."customer_invoice_lottery SET
	...
	public function updateStatusPD($pd_id , $status,$url){
		$this -> db -> query("UPDATE " . DB_PREFIX . "customer_provide_donation_lottery SET 
			status = '".$status."',
			url = '".$url."'
			WHERE id = '".$pd_id."'
		");
	...
	public function check_confirm_btc($pd_id){
		$query = $this -> db -> query("
			SELECT bet_numbers,status
			FROM ". DB_PREFIX . "customer_provide_donation_lottery
			WHERE id = '".$pd_id."' 
		");
		return $query -> row;
